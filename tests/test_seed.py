...
import models
from seed import run_seed, _seed_admin, _seed_config, _generate_secure_password
...
class TestSeedAdmin:
    def test_creates_admin_when_no_users(self, db):
    ...
    def test_admin_username_from_env(self, db, monkeypatch):
    ...
    def test_admin_name_from_env(self, db, monkeypatch):
    ...
    def test_admin_password_from_env(self, db, monkeypatch):
    ...
    def test_generates_password_when_not_set(self, db, monkeypatch):
        """Si ADMIN_PASSWORD no esta definida, genera una contraseña segura."""
    ...
    def test_idempotent_does_not_create_duplicate(self, db, monkeypatch):
    ...
    def test_skip_if_users_exist(self, db):
    ...
    def test_default_username_is_admin(self, db, monkeypatch):
    ...
    def test_default_name_is_administrador(self, db, monkeypatch):
...
class TestSeedConfig:
    def test_creates_config_when_none_exists(self, db):
    ...
    def test_idempotent_does_not_create_duplicate(self, db):
        """Llamar dos veces no crea una segunda configuracion."""
        _seed_config(db)
        _seed_config(db)
        assert db.query(models.Configuracion).count() == 1

    def test_skip_if_config_exists(self, db):
